...
export const DashboardView: React.FC<DashboardViewProps> = ({
  ...
  // Ingestion panel render function (rendered whether data exists or not)
  const renderIngestionPanel = () => (
    <div id="dashboard-file-upload-panel" className={`border rounded p-3.5 space-y-3 transition-colors ${
        ...
        <div>
          <div className="flex items-center gap-2">
            ...
            <span className={`text-[10px] font-mono px-1.5 py-0.2 rounded border font-semibold ${
              ...
              Up to 1 TB Streaming
            </span>
          </div>
          <p className={`text-[11px] mt-0.5 ${isLight ? 'text-[#475569]' : 'text-[#94A3B8]'}`}>
            Upload any cryptographic payload, signature bundle, or PEM certificate directly here for immediate line-level forensic triage.
          </p>
        </div>
        ...
        <div className="flex items-center gap-2">
          {onModeChange && (
            <select
              ...
              className={`text-xs rounded px-2.5 py-1 font-mono border focus:outline-none transition cursor-pointer ${
      ...
      {/* Streaming Upload Progress Bar (Active when uploading) */}
      {uploadProgress && (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-3">
        ...
        <div
          ...
          className={`lg:col-span-8 border-2 border-dashed rounded p-3 text-center cursor-pointer transition flex items-center justify-center gap-3 ${
            isDragging
        ...
        >
          <input
            ...
            className="hidden"
          />
          <Upload className={`w-5 h-5 shrink-0 ${isLight ? 'text-[#0284C7]' : 'text-[#38BDF8]'}`} />
          <div className="text-left">
            <div className={`text-xs font-bold font-mono ${isLight ? 'text-[#0F172A]' : 'text-[#F1F5F9]'}`}>
              {selectedFile ? `Selected: ${selectedFile.name}` : 'Drop security artifact here or click to browse'}
            </div>
            <div className={`text-[11px] font-mono ${isLight ? 'text-[#64748B]' : 'text-[#94A3B8]'}`}>
              Supports .txt, .json, .pem, .sig, .bin &bull; Streaming pipeline up to 1 TB
            </div>
          </div>
        </div>

        {/* Action button & Presets column (4 cols) */}
        <div className="lg:col-span-4 flex flex-col justify-between gap-2">
          ...
          <div className="flex flex-wrap items-center gap-1 text-[11px]">
            <span className={`text-[10px] font-mono uppercase font-semibold mr-1 ${isLight ? 'text-[#64748B]' : 'text-[#64748B]'}`}>
              Presets:
            </span>
            {[
              ...
              { id: 'test_6_quantum_eavesdropping.txt', label: '6: Quantum' }
            ].map(s => (
              <button
                key={s.id}
                ...
                className={`px-2 py-0.5 rounded text-[10px] font-mono border transition cursor-pointer ${
                  activeSampleId === s.id && !selectedFile
                    ...
                    : isLight
                      ...
                      : 'bg-[#162032] border-[#1E293B] text-[#94A3B8] hover:bg-[#1E293B]'
                }`}
              >
                {s.label}
              </button>
  ...
  // If no data is available yet, render Ingestion panel + prompt
  if (!data) {
    return (
      <div className="space-y-4">
        {renderIngestionPanel()}
        <div className={`p-8 border rounded-lg text-center space-y-3 ${
          isLight ? 'bg-white border-[#CBD5E1]' : 'bg-[#111827] border-[#1E293B]'
        }`}>
          <div className="w-12 h-12 rounded-full mx-auto flex items-center justify-center bg-[#0284C7]/15 text-[#0284C7]">
            <Shield className="w-6 h-6" />
          </div>
          <h3 className={`text-base font-bold ${isLight ? 'text-[#0F172A]' : 'text-[#F1F5F9]'}`}>
            Ready to Ingest &amp; Analyze Security Artifacts
          </h3>
          <p className={`text-xs max-w-md mx-auto ${isLight ? 'text-[#475569]' : 'text-[#94A3B8]'}`}>
            Drop a payload or PEM certificate in the upload zone above, or select a preset sample to view line-level forensic evidence and quantum channel metrics.
          </p>
        </div>
      </div>
    );
  ...
  const threatOverviewItems = [
    {
      ...
      detected: (data.attack_table || []).some(r => r.attack === 'Forgery' && (r.status !== 'NOT DETECTED')) ||
        data.signature?.signature_status?.toUpperCase().includes('INVALID') ||
    ...
    },
    {
      ...
      detected: (data.attack_table || []).some(r => r.attack === 'Certificate / Trust Failure' && (r.status !== 'NOT DETECTED')) ||
        ...
        data.threat?.detected_threat?.toLowerCase().includes('certificate'),
    ...
    },
    {
      ...
      detected: Boolean(data.signature?.hash_mismatch) ||
        data.file?.integrity_status?.toUpperCase().includes('MISMATCH') ||
    ...
    },
    {
      ...
      detected: (data.attack_table || []).some(r => r.attack === 'Entangle-and-Measure' && (r.status !== 'NOT DETECTED')) ||
        ...
        data.threat?.detected_threat?.toLowerCase().includes('eavesdrop') ||
    ...
    },
    {
      ...
      detected: (data.attack_table || []).some(r => r.attack === 'Intercept-Resend' && (r.status !== 'NOT DETECTED')) ||
        data.threat?.detected_threat?.toLowerCase().includes('intercept'),
  ...
  return (
    <div className="space-y-4">
      {/* 1. DIRECT DASHBOARD FILE INGESTION & UPLOAD PANEL */}
      {renderIngestionPanel()}

      {/* 2. TOP SUMMARY METRIC CARDS (Compact SOC Design) */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        <div className={`border rounded p-3 flex flex-col justify-between transition-colors ${
          ...
          <div className={`text-[11px] mt-2 font-mono truncate ${isLight ? 'text-[#475569]' : 'text-[#64748B]'}`} title={data.file?.filename}>
            Target: <span className={`font-semibold ${isLight ? 'text-[#0F172A]' : 'text-[#94A3B8]'}`}>{data.file?.filename || 'None'}</span>
          </div>
        </div>
      ...
      {/* 3. ATTACK EVIDENCE LOCATION & LINE-LEVEL FORENSIC ANALYSIS */}
      <div id="dashboard-attack-evidence-location">
        <EvidenceForensicViewer
          evidence={data.evidence || data.threat?.evidence_items || []}
          timeline={data.evidence_timeline}
          fileName={data.file?.filename || selectedFile?.name || 'artifact.txt'}
          fileSize={data.file?.file_size || (selectedFile ? `${Math.round(selectedFile.size / 1024)} KB` : undefined)}
          isSecure={data.threat?.status === 'SECURE' || riskScore < 35}
          theme={theme}
        />
      </div>

      {/* 4. MAIN 2-COLUMN SECTION: SOC THREAT TABLE & MULTI-THREAT DISTRIBUTION */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
        {/* Left: SOC-Style Threat Table (7 cols) */}
        ...
        {/* Right: Multi-Threat Distribution & Quick Case Summary (5 cols) */}
        <div className="lg:col-span-5 space-y-4">
          ...
          <div className={`border rounded p-3.5 space-y-3 transition-colors ${
            ...
            <div className={`pt-1 text-[11px] flex items-center justify-between border-t font-mono ${
              ...
              <span>Threat Level: <strong className={isLight ? 'text-[#0F172A]' : 'text-[#F1F5F9]'}>{data.threat?.risk}</strong></span>
              <span>Confidence: <strong className={isLight ? 'text-[#0284C7]' : 'text-[#38BDF8]'}>{data.threat?.confidence}%</strong></span>
            </div>
          </div>
          ...
          <div className={`border rounded p-3.5 space-y-2 text-xs transition-colors ${
            ...
            <div className="space-y-1 font-mono text-[11px]">
              <div className={`flex justify-between py-1 border-b ${isLight ? 'border-[#E2E8F0]' : 'border-[#1E293B]'}`}>
                ...
                <span className={`font-semibold ${isLight ? 'text-[#0F172A]' : 'text-[#F1F5F9]'}`}>{data.signature?.signature_algorithm || 'ECDSA / RSA'}</span>
              </div>
              <div className={`flex justify-between py-1 border-b ${isLight ? 'border-[#E2E8F0]' : 'border-[#1E293B]'}`}>
                ...
                <span className={data.signature?.signature_status === 'VALID' 
                  ...
                  {data.signature?.signature_status || 'UNKNOWN'}
                </span>
              </div>
              <div className={`flex justify-between py-1 border-b ${isLight ? 'border-[#E2E8F0]' : 'border-[#1E293B]'}`}>
      ...
      {/* 3. ANALYSIS HISTORY TABLE (SOC Case Ledger) */}
      <div className={`border rounded p-3.5 space-y-3 transition-colors ${
        isLight ? 'bg-white border-[#CBD5E1] shadow-xs' : 'bg-[#111827] border-[#1E293B]'
      }`}>
        <div className={`flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b pb-2 ${
